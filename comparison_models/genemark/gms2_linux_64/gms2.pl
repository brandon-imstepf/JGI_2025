...
sub GetTrainingCommand {
    ...
    # Training step: use prediction of previous iteration
    my $trainingCommand = "$trainer gms2-training -s $fnseq -l $prevPred -m $currMod --order-coding $orderCod --order-noncoding $orderNon --only-train-on-native $nativeOnly --genetic-code $geneticCode --order-start-context $scOrder --fgio-dist-thr $fgioDistThresh --cpus $threads";
...
--threads                               Number of threads (CPUs) to use in Gibbs sampler search. (default: $threads)
