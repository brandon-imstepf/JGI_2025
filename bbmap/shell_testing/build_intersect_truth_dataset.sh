...
----------------------------------
Generate training TSVs where "truth" = intersection(CallGenes filtered predictions, NCBI reference).
Per genome, the pipeline:
  1) Runs CallGenes twice (nofilter + filtered)
  2) Positives = filtered ∩ reference
  3) Negatives = all candidates \ (filtered ∪ reference)
No subsampling is applied; every candidate is preserved.
Outputs per run:
  - Combined TSV (positives + negatives)
  - Aggregate positives-only / negatives-only TSVs
  - Per-genome TSVs under positives/ and negatives/ folders
...
for ref_fasta in "${INFOLDER}"/*.fna.gz; do
    ...
    if ! "${CALLGENES}" in="${ref_fasta}" outgff="${PRED_GFF}" truegenes="${ref_gff}" cds "${PASSTHRU_ARGS[@]}" >/dev/null; then
        echo "[ERROR] CallGenes filtered run failed for ${base}; skipping genome" >&2
        continue
    fi

    if ! "${GFFSETOP}" in_a="${PRED_GFF}" in_b="${ref_gff}" out="${UNION_GFF}" op=union >/dev/null; then
        echo "[ERROR] GffSetOperator union failed for ${base}; skipping genome" >&2
        continue
    fi
    if ! "${GFFSETOP}" in_a="${ALL_GFF}" in_b="${UNION_GFF}" out="${NEG_GFF}" op=subtract >/dev/null; then
        echo "[ERROR] GffSetOperator subtract failed for ${base}; skipping genome" >&2
        continue
    fi
    if ! "${GFFSETOP}" in_a="${PRED_GFF}" in_b="${ref_gff}" out="${POS_GFF}" op=intersect >/dev/null; then
    ...
    rm -f "${ALL_GFF}" "${PRED_GFF}" "${UNION_GFF}" "${POS_GFF}" "${NEG_GFF}"
done
