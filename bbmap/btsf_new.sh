...
for ref_fasta in "$INFOLDER"/*.fna.gz; do
    ...
    # Negatives = All candidates that are NOT in the NCBI reference.
    gffsetop.sh in_a="$ALL_GFF" in_b="$ref_gff" out="$NEGATIVES_GFF" op=subtract
    # Positives = All candidates that ARE in the NCBI reference.
    gffsetop.sh in_a="$ALL_GFF" in_b="$ref_gff" out="$POSITIVES_GFF" op=intersect
    ...
    # --- Per-Genome Balancing ---
    echo "    Balancing negatives for $base..."
    POS_COUNT_GENOME=$(wc -l < "$TMPDIR/${base}.positives.tsv" 2>/dev/null || echo 0)
    NEG_COUNT_GENOME=$(wc -l < "$TMPDIR/${base}.negatives.tsv" 2>/dev/null || echo 0)
    MAX_NEG_GENOME=$((POS_COUNT_GENOME * 2))

    echo "      Positives: $POS_COUNT_GENOME, Negatives: $NEG_COUNT_GENOME, Target Negatives: $MAX_NEG_GENOME"

    if (( NEG_COUNT_GENOME > MAX_NEG_GENOME )); then
        echo "      Subsampling negatives from $NEG_COUNT_GENOME to $MAX_NEG_GENOME..."
        # Shuffle the negatives file and overwrite it with the subsampled version
        shuf -n "$MAX_NEG_GENOME" "$TMPDIR/${base}.negatives.tsv" > "$TMPDIR/${base}.negatives.balanced.tsv"
        mv "$TMPDIR/${base}.negatives.balanced.tsv" "$TMPDIR/${base}.negatives.tsv"
    fi
    # --- End Per-Genome Balancing ---
...
# Concatenate all the pre-balanced TSV files
echo "Concatenating final TSV files..."
