...
public final class MSAViz implements Aligner, IDAligner {
	...
	public static void main(String[] args) {
		...
		MSAViz msa=new MSAViz();
		
		if(args.length>2) {msa.output=args[2];}
		if("null".equalsIgnoreCase(msa.output)) {msa.output=null;}
		...
		Timer t=new Timer();
		float id=0;
		for(int i=0; i<iters; i++) {id=msa.align(a, b, pos, 0, b.length-1, 15*a.length);}
		long loops=msa.loops();
		t.stopAndStart("Identity MSA:       \t"+id+"\tcoords=("+pos[0]+", "+pos[1]+")"
	...
	@Override
	public float align(byte[] q, byte[] r, int[] posVector, int minScore) {return align(q, r, posVector, 0, r.length-1, minScore);}
	@Override
	...
	public float align(byte[] q, byte[] r, int[] pos, int from, int to, int minScore) {
		...
		int[] max=fillLimited(q, r, from, to, minScore);
...
//			}
	...
	private final int[] fillLimitedX(byte[] read, byte[] ref, int refStartLoc, int refEndLoc, int minScore){
		...
		Visualizer viz=(output==null ? null : new Visualizer(output, SCOREOFFSET, 0));
		
		minScore-=100; //Increases quality trivially
		
//		assert(false) : minScore+", "+viz;
...
//					assert((prevState&MODEMASK)==prevState);
			...
			if(viz!=null) {viz.printMSA(packed[MODE_MS][row], read.length, ref.length, POINTS_MATCH);}
		}
