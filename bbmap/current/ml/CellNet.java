...
//		weightedErrorSum+=s.weightedErrorMagnitude;
	...
	public void backPropDense(float[] truth, float weightMult) {
		...
		//assert(check());
		if(normalization_factor>0.0001f) {
			if(NORMALIZE_BIAS) {normalizeBiasDense();}
			if(NORMALIZE_WEIGHTS) {normalizeWeightsDense();}
			normalization_factor*=normalization_shrink_rate;
		}
	}
	
	/**
	 * Normalizes the biases of the dense layers in the network.
	 */
	public void normalizeBiasDense() {
		...
		for(int lnum=layers-2; lnum>0; lnum--) {
			...
			float mean=(float)list.mean();
			float stdev=list.stdev();
			if(stdev<0.001f) {stdev=0.001f;}//Avoid division by zero
			float invStdev=1f/stdev;
			for(int i=0; i<layer.length; i++){
				Cell c=layer[i];
				float f=c.bias;
				float target=(f-mean)*invStdev;
				f=(1f-normalization_factor)*f+normalization_factor*target;
				c.bias=f;
			}
		}
	}
	...
	public void normalizeWeightsDense() {
		...
		for(int layer=0; layer<weightsIn.length-1; layer++) {
			float[][] weightsInL=weightsIn[layer+1];
			normalizeDense(weightsInL, list);
		}
		transposeDense();
	}
		...
		for(float[] array : weights) {
			for(int i=0; i<array.length; i++) {
				...
				if(f!=0) {
					float target=(f-mean)*invStdev;
					f=(1f-normalization_factor)*f+normalization_factor*target;
					array[i]=(Math.abs(f)<1e-12f) ? 1e-12f : f;
				}
			}
		}
	}
