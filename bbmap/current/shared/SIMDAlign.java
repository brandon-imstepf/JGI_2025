...
//		int alignedEnd = bandStart + ((bandEnd - bandStart + 1)/LWIDTH)*LWIDTH;
//
//		// Broadcast constants to vectors
//		LongVector vq = LongVector.broadcast(LSPECIES, q);
//		LongVector vMATCH = LongVector.broadcast(LSPECIES, MATCH);
//		LongVector vN_SCORE = LongVector.broadcast(LSPECIES, N_SCORE);
//		LongVector vSUB = LongVector.broadcast(LSPECIES, SUB);
//		LongVector vINS = LongVector.broadcast(LSPECIES, INS);
//
//		// Process in LWIDTH chunks
//		for(int j=alignedStart; j<alignedEnd; j+=LWIDTH) {
//			// Load reference values
//			LongVector refVec = LongVector.fromArray(LSPECIES, ref, j-1);
//
//			// Calculate match conditions with 4-bit encoding
//			LongVector andResult = vq.lanewise(VectorOperators.AND, refVec);
//			VectorMask<Long> isCompatible = andResult.compare(VectorOperators.GT, 0);
//			VectorMask<Long> isEqual = vq.compare(VectorOperators.EQ, refVec);
//			VectorMask<Long> isMatchMask = isEqual.and(isCompatible);
//
//			// Check for N values
//			VectorMask<Long> qIsN = vq.compare(VectorOperators.EQ, 15);
//			VectorMask<Long> refIsN = refVec.compare(VectorOperators.EQ, 31);
//			VectorMask<Long> hasNMask = qIsN.or(refIsN);
//
//			// Start with SUB score for all positions
//			LongVector scoreAddVec = vSUB;
//
...
//	}
	...
	public static void alignBandVector_old(long q, long[] ref, int bandStart, int bandEnd, 
	        long[] prev, long[] curr, 
	        long MATCH, long N_SCORE, long SUB, long INS){

	    // Calculate vector-aligned bounds; does not properly handle bandStart, but is correct
	    final int alignedStart=bandStart;
	    final int alignedEnd0=bandStart+((bandEnd-bandStart+1)/LWIDTH)*LWIDTH;
	    final int alignedEnd=(((alignedEnd0&3)==0 || 
	    		ref.length<alignedEnd0+3 ? alignedEnd0 : (alignedEnd0+3)&~3));
	    
//	 // Calculate vector-aligned bounds to maximize vector usage when padding allows
//	    final int alignedStart=bandStart;//Not needed
//	    final int rangeSize = bandEnd - bandStart + 1;
//	    final int completeVectors = rangeSize / LWIDTH;
//	    final int remainingElements = rangeSize % LWIDTH;
//
//	    // Check if we can safely do one more vector iteration with the available padding
//	    final boolean needsExtraVector = remainingElements > 0;
//	    final boolean canFitExtraVector = (bandStart + completeVectors * LWIDTH + LWIDTH - 1) < ref.length;
//
//	    final int alignedEnd = needsExtraVector && canFitExtraVector ? 
//	                          bandStart + (completeVectors + 1) * LWIDTH : 
//	                          bandStart + completeVectors * LWIDTH;


	    // Broadcast constants to vectors - with additional constants pre-computed
	    ...
	    final LongVector v31=LongVector.broadcast(LSPECIES, 31);
	    final LongVector v0=LongVector.broadcast(LSPECIES, 0);

	    // Process in LWIDTH chunks - no unrolling
	    for(int j=alignedStart; j<alignedEnd; j+=LWIDTH){
	        // Load reference values
	        LongVector refVec=LongVector.fromArray(LSPECIES, ref, j-1);

	        // Simplified match condition calculation in one step
	        VectorMask<Long> isMatchMask=vq.compare(VectorOperators.EQ, refVec)
	                .and(vq.lanewise(VectorOperators.AND, refVec).compare(VectorOperators.GT, v0));
	        
	        // Directly check for N values using pre-computed constants
	        VectorMask<Long> hasNMask=vq.compare(VectorOperators.EQ, v15)
	                .or(refVec.compare(VectorOperators.EQ, v31));

	        // Combined blend operations - apply both conditions in a chain
	    ...
	    // Handle remaining elements (scalar code) - unchanged
	    for(int j=alignedEnd; j<=bandEnd; j++){
	        final long r=ref[j-1];
	        final boolean isMatch=((q&r)>0 && q==r);
	        final boolean hasN=(q==15 || r==31);
	...
	public static void alignBandVectorDel(long q, long[] ref, int bandStart, int bandEnd, 
	        long[] prev, long[] curr, 
	        long MATCH, long N_SCORE, long SUB, long INS, long DEL){
	    ...
	    if(rangeSize>=LWIDTH){
	        ...
	        for(int j=bandStart; j<bandEnd-(LWIDTH-2); j+=LWIDTH){
	            ...
	            long left=curr[j-1]+DEL;
	            curr[j]=left=Math.max(left, curr[j]);
	            curr[j+1]=left=Math.max(left, curr[j+1]);
	            curr[j+2]=left=Math.max(left, curr[j+2]);
	            curr[j+3]=left=Math.max(left, curr[j+3]);
	        }
...
//            curr[j+3]=left=((left&MASK)>curr[j+3] ? left : curr[j+3]);
		...
		if(rangeSize>=LWIDTH) {// If range is at least one vector wide
			...
			for(int j=bandStart; j<bandEnd-(LWIDTH-2); j+=LWIDTH) {
				...
				VectorMask<Long> isMatchMask=vq.compare(VectorOperators.EQ, refVec)
						.and(vq.lanewise(VectorOperators.AND, refVec).compare(VectorOperators.GT, v0));
				VectorMask<Long> hasNMask=vq.compare(VectorOperators.EQ, v15)
						.or(refVec.compare(VectorOperators.EQ, v31));// Check for N values
				// Combined blend operations
				LongVector scoreAddVec=vSUB.blend(vMATCH, isMatchMask).blend(vN_SCORE, hasNMask);
				// Load previous scores
				LongVector prevJ1Vec=LongVector.fromArray(LSPECIES, prev, j-1);
	...
	public static void alignBandVector2(long q, long[] ref, int bandStart, int bandEnd, 
			long[] prev, long[] curr, 
			long MATCH, long N_SCORE, long SUB, long INS) {
		...
		// Create broadcast vectors for constants
		LongVector vq = LongVector.broadcast(LSPECIES, q);
		LongVector vMATCH = LongVector.broadcast(LSPECIES, MATCH);
		LongVector vN_SCORE = LongVector.broadcast(LSPECIES, N_SCORE);
		LongVector vSUB = LongVector.broadcast(LSPECIES, SUB);
		LongVector vINS = LongVector.broadcast(LSPECIES, INS);
		LongVector v15 = LongVector.broadcast(LSPECIES, 15);

		// Process in LWIDTH chunks with prefetching
		for(int j=alignedStart; j<alignedEnd; j+=LWIDTH) {
			// Prefetch next data (helps with memory latency)
//			if(j+LWIDTH*2 <= alignedEnd) {
//				LSPECIES.prefetch(ref, j+LWIDTH-1);
//				LSPECIES.prefetch(prev, j+LWIDTH-1);
//			}

			// Load reference values
			LongVector refVec = LongVector.fromArray(LSPECIES, ref, j-1);

			// Calculate match condition (exact equality)
			VectorMask<Long> isMatchMask = vq.compare(VectorOperators.EQ, refVec);

			// Calculate hasN using OR operation: (q|r)>=15
			LongVector orResult = vq.lanewise(VectorOperators.OR, refVec);
			VectorMask<Long> hasNMask = orResult.compare(VectorOperators.GE, v15);

			// Load previous scores
			LongVector prevJ1Vec = LongVector.fromArray(LSPECIES, prev, j-1);
			LongVector prevJVec = LongVector.fromArray(LSPECIES, prev, j);

			// Compute score additions efficiently
			// Start with SUB scores
			LongVector scoreAddVec = vSUB;

			// Apply N_SCORE where hasN is true (but not where isMatch is true)
			VectorMask<Long> useNScoreMask = hasNMask.andNot(isMatchMask);
			scoreAddVec = scoreAddVec.blend(vN_SCORE, useNScoreMask);

			// Apply MATCH where isMatch is true (overrides everything)
			scoreAddVec = scoreAddVec.blend(vMATCH, isMatchMask);
			...
			// Find max and store
			diagScoreVec.max(upScoreVec).intoArray(curr, j);
		}
...
//	    print(curr, "Initial");
	    ...
	    // Broadcast constant
	    final LongVector delIncrementVec = LongVector.broadcast(LSPECIES, DEL_INCREMENT);
...
//	    if(limit>bandStart) {System.err.println("Vector processing "+bandStart+" to "+(limit-1));}
	    for(int i=bandStart; i<limit; i+=LWIDTH) {
	    	for(int pass=0, dist=LWIDTH/2; dist>0 && i-dist>=0; pass++, dist/=2) {
	    	    ...
	    	    LongVector adjustedDelIncrement = LongVector.broadcast(LSPECIES, DEL_INCREMENT * dist);
...
//	    System.err.println();
	...
	public static void processInnerDiagonalVectorized(
	        ...
	        int qLen, boolean debug, long MATCH, long N_SCORE, long SUB,
	...
	public static void processCrossCutDiagonalSIMD_old(
	        ...
	        long N_SCORE, long SCORE_MASK, boolean debug) {
	    
	    if(debug) {System.err.print(".");}
	    
	    // Calculate the number of vector operations needed
	    final int vectorBound=innerMaxCol-LWIDTH+1;
	    
	    // Flag to track if any SIMD processing occurred
	    boolean didSIMD=false;
	    ...
	    // Handle vector-sized blocks
	    for(int col=innerMinCol; col<=vectorBound; col+=LWIDTH) {
	        didSIMD=true;
	        
	     // Add at the start of processInnerLoopSIMD
//	        System.out.println("SIMD processing diagonal k=" + k + 
//	                           " from col=" + innerMinCol + " to " + innerMaxCol);
...
//	        final LongVector maskedLeftVector=leftScoreVector.and(scoreMaskVector);
//	        final VectorMask<Long> comparisonMask=maskedMaxVector.compare(
//	                VectorOperators.GE, maskedLeftVector);
...
//	        // Unmasked comparison
//	        final VectorMask<Long> comparisonMask=maxDiagUpVector.compare(
//	                VectorOperators.GE, leftScoreVector);
	        
	        // Select final max - CORRECT BLENDING
	        final LongVector resultVector=leftScoreVector.blend(maxDiagUpVector, comparisonMask);
	        
	        // Store results directly - aligned, contiguous store
	    ...
	    int startCol;
	    if(didSIMD) {
	        // Process ONLY columns that aren't handled by complete LWIDTH-sized blocks
	        // The last full vector block ends at: innerMinCol + (numFullBlocks * LWIDTH) - 1
	        int numFullBlocks = (innerMaxCol - innerMinCol + 1) / LWIDTH;
	        startCol = innerMinCol + (numFullBlocks * LWIDTH);
	    } else {
	        // If no SIMD processing occurred, process all cells using scalar code
	        startCol = innerMinCol;
	    }
	    
	    
	    for(int col=startCol; col<=innerMaxCol; col++) {
	        int row=k-col;
	        
	        if(row < 2 || row > qLen) continue; // Skip invalid rows
//            System.out.println("Scalar processing (k=" + k + "): cell [" + row + "," + col + "]");
