...
public class Shared {
	...
	private static String demuxServer="https://demux.jgi.doe.gov/";//port 4096 on taxvm2
...
//				e.printStackTrace();
	...
	public static String BBTOOLS_VERSION_STRING="39.26";
	public static String BBMAP_VERSION_NAME="Isla Intervention";
...
//		assert(false) : READ_BUFFER_MAX_DATA+" -> "+x; //TODO: 123
	...
	public static final void printMemory(){
		try{
			...
			System.err.println("Memory: "+"max="+mmemory+"m, total="+tmemory+"m, "+
...
//			randy=new Random();
//		}
		return new FastRandom(seed);//Similar speed but safer
	}
	...
	public static final <T extends Comparable<? super T>> void sort(T[] array, int from, int to, Comparator<? super T> comparator){
		try {
			if(!parallelSort || array.length<=parallelSortLength || THREADS<2){
				Arrays.sort(array, from, to, comparator); //Supported in pre-Java 8.
				return;
			}
