...
public class GeneCaller extends ProkObject {
	...
	// New field
    private boolean generateAllCandidates = false;

    // New public method (a "setter")
    public void setGenerateAllCandidates(boolean b) {
        this.generateAllCandidates = b;
	}
	...
	public ArrayList<Orf> callGenes(Read r, GeneModel pgm_, boolean breakOrfs){
		...
		// Brandon: if generateAllCandidates is true, return all candidates without any filtering
		// ---- START MODIFICATION ----
        if (this.generateAllCandidates) {
            ArrayList<Orf> allCandidates = new ArrayList<Orf>();
            for (ArrayList<Orf> list : brokenLists) {
                if (list != null) {
                    allCandidates.addAll(list);
                }
            }
            // Optional: Sorting helps keep the output deterministic if you need that
...
//					outstream.println(x);
		...
		// Brandon: Generate all candidates
  		// ---- START MODIFICATION ----
        if (!generateAllCandidates) {
            //Sort to by score descending to eliminate low-scoring copies.
            if(broken.size()>1){Collections.sort(broken, PFeature.featureComparatorScore);}

            int removed=0;
            for(int i=0; i<size; i++){//Fix scores because they were generated together...
                Orf orf=broken.get(i);
		...
		geneStopsRetained+=( !broken.isEmpty() ? 1 : 0);
