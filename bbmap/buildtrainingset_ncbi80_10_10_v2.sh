#!/bin/bash
# buildtrainingset_ncbi80_10_10_v2.sh
#
...
#
# Usage: buildtrainingset_ncbi80_10_10_v2.sh in=<folder> out=<master.tsv> [options]
# Options:
...
\nExamples:
  buildtrainingset_ncbi80_10_10_v2.sh in=./data out=master.tsv seed=123
USAGE
}
...
for ref_fasta in "$INFOLDER"/*.fna.gz; do
  ...
  # Save first few FASTA contig IDs for comparison
  echo "FASTA contig IDs for $base:" >&2
  zcat "$ref_fasta" | awk 'BEGIN{RS=">";FS="\n"} NR>1{split($1,a,/\s+/); print a[1]}' | head -n 3 >&2

  # Create contig lengths for this genome from fasta
  # Produce lines: contig<TAB>length<TAB>genome_base
  # Normalize the contig id by trimming at first whitespace
  ...
  # Process and append NCBI GFF — don't alter file formatting (Java parser expects strict GFF)
  if [[ "$ref_gff" == *.gz ]]; then
    echo "NCBI GFF contig IDs for $base:" >&2
    zcat "$ref_gff" | awk '!/^#/ && NF>8 {split($1,a,/\s+/); print a[1]}' | sort -u | head -n 3 >&2
    # Append raw NCBI GFF content unchanged
    zcat "$ref_gff" >> "$NCBI_ALL_GFF"
  else
    echo "NCBI GFF contig IDs for $base:" >&2
    awk '!/^#/ && NF>8 {split($1,a,/\s+/); print a[1]}' "$ref_gff" | sort -u | head -n 3 >&2
...
# Print sample of contig_lengths.tsv for verification
echo "Sample entries from contig_lengths.tsv:" >&2
head -n 3 "$CONTIG_LENGTHS" >&2

echo "Building NCBI interval mask (BED)"
# Convert NCBI GFF -> BED (0-based, half-open). Keep only gene/CDS lines
# Ensure contig IDs are normalized consistently with above
awk '$0!~/^#/ && NF>8 {split($1,a,/\s+/); $1=a[1]; print $1"\t"($4-1)"\t"$5"\t"$3}' "$NCBI_ALL_GFF" > "$NCBI_BED"

echo "Filtering all ORFs against global NCBI gene set to make negatives"
# Use gffsetop.sh to subtract NCBI genes from the full ORF set
...
# Print key file stats before splitting
echo "Lines in key files before splitting:" >&2
wc -l "$NCBI_ALL_GFF" "$ALL_ORFS_GLOBAL" "$TMPDIR/all_orfs_no_ncbi_overlap.gff" 2>/dev/null || true
...
def route_gff(infile, outprefix):
    ...
    for line in open(infile):
        ...
        contig=line.split('\t')[0].split()[0]  # normalize contig ID here too
        s=split.get(contig)
        if s:
            outs[s].write(line)
...
# Print counts of split files
echo "Lines in split files:" >&2
wc -l "$NCBI_TRAIN" "$NCBI_VAL" "$NCBI_TEST" "$NEG_TRAIN" "$NEG_VAL" "$NEG_TEST" 2>/dev/null || true
...
echo "# Combined training set produced by buildtrainingset_ncbi80_10_10_v2.sh" > "$OUT_TSV"
